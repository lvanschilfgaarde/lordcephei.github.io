...
In order to run a DFT calculation, you need an input file and structural information. The **blm**{: style="color: blue"} tool takes the init file as input and creates a template input file **actrl.si** and structure file **site.si**. Note that the code recognises certain names as file types (such as "ctrl" for input file and "site" for structure file).  Extension ("si") labels the materials system; it is something you choose.  Most files generated by the Questaal package will use this extension. The prefix "a" in **actrl.si** is used to prevent overwriting of an existing ctrl file. 
