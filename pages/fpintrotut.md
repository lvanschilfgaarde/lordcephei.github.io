...
Or alternatively copy cut and paste the box's contents to _init.si_{: style="color: green"}.
...
Si forms in the diamond cubic structure, space group 227 as you can easily find e.g. on[ this web page](http://www.periodictable.com/Properties/A/SpaceGroupNumber.html).
Space group 227 is Fd-3m (you can equally well have entered **SPCGRP=Fd-3m**) which has an underlying fcc Bravais lattice

This fact, knowledge of the lattice constant and the position of the basis vectors in **SITE** is all that is needed to fix the structural information. In the 
**SITE** category, the atomic number is inferred from the symbol (Si). Tag **X=** specifies that the coordinates are in "direct" representation, that is, as fractional multiples of lattice vectors. It doesn't matter in this case, but it is also possible to use Cartesian coordinates : use **POS=** instead of **X** (see additional exercises below).  Positions given in Cartesian coordinates are in units of **ALAT**, as are the lattice vectors.

In order to run a DFT calculation, you need an input file along with structural information. The **blm**{: style="color: blue"} tool read the init file as input and creates a template input file _actrl.si_{: style="color: green"} and structure file _site.si_{: style="color: green"}. Note that the Questaal package recognises certain names as file types (such as "ctrl" for input file and "site" for structure file). The prefix "a" in _actrl.si_{: style="color: green"} is used to prevent overwriting of an existing ctrl file. Extension "si" labels the materials system; it is something you choose.  Most files read by or generated by Questaal programs will append this extension.  
