...
Check the contents of your working directory and you will find two new files **atm.si**{: style="color: green"}  and **basp0.si**{: style="color: green"} . **atm.si**{: style="color: green"}  contains the free atom densities calculated by **lmfa**{: style="color: blue"}. File **basp0.si**{: style="color: green"}  is the template basis set file; the standard basis set name is basp and the extra 0 is appended to avoid overwriting. Take a look at **basp0.si**{: style="color: green"}  and you will see that it contains basis set parameters that define silicon's smooth Hankel functions. Changing these values would change their functional form, but **lmfa**{: style="color: blue"} does a reasonable job (also later on parameters can be automatically optimized, if desired) so we will leave them as they are. Copy **basp0.si**{: style="color: green"}  to **basp.si**{: style="color: green"}, which is the name **lmf** recognises for the basis set file
