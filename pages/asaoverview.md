...
+ **lmchk**{: style="color: blue"} will run the algorithm and print the results to stdout if you invoke it with `lmchk --getwsr ...`\\
  ...
  There is at present no automatic way to incorporate radii generated by lmchk into the input system.
...
you can pack the volume with "atoms" with
...
_Note:_{: style="color: red"} In this instance <i>P<sub>l</sub></i> Andersen's "potential function;"
not the "continuously varying principal quantum number!
1/P is the LMTO analogue of the tangent of the phase shift in multiple scattering and KKR theories.
