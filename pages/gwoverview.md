...
"quasiparticlized" approximation $$V_{xc}$$ to $$\Sigma_{ij}(\omega)$,
and defines a one-particle hamiltonian $$H_0$$ which is the
LDA one-particle hamiltonian with $$V^{GW}_{xc}$$ substituting for $$V^{LDA}_{xc}$$.
...
In reality it makes $$V^{GW}_{xc}{-}V^{LDA}_{xc}$$.  **lmf**{: style="color: blue"} reads this potential and adds it to the LDA hamiltonian.
